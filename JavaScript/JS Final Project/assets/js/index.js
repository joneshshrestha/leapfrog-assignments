var dhtmlwindow = {
  ...
  init: function (t) {
    ...
    domwindowdata +=
      'Window <div class="drag-controls"><img src="' +
      this.imagefiles[0] +
      '" class="NavButton" title="Minimize" /><img src="' +
      this.imagefiles[4] +
      '" title="Maximize" class="NavButton" /><img src="' +
      this.imagefiles[1] +
      '" class="ExitButton" title="Close" /></div>';
    domwindowdata += "</div>";
  ...
  load: function (t, contenttype, contentsource, title) {
    ...
    if (t.isClosed) {
      alert(
        "DHTML Window has been closed, so no window to load contents into. Open/Create the window again."
      );
      return;
    }
    ...
    if (contenttype == "inline") t.contentarea.innerHTML = contentsource;
      ...
      t.contentarea.innerHTML =
        inlinedivref.defaultHTML || inlinedivref.innerHTML; //Populate window with contents of inline div on page
      ...
      if (
        ...
        t.contentarea.innerHTML =
          '<iframe src="" frameborder="0" style="frameborder:0; margin:0; padding:0; width:100%; height: 100%" name="_iframe-' +
          t.id +
          '"></iframe>';
  ...
  setupdrag: function (e) {
    ...
    if (t.contentarea.datatype == "iframe") {
      ...
      t.style.backgroundColor = "white"; //colorize and hide content div (while window is being dragged)
  ...
  getdistance: function (e) {
    ...
    if (etarget.className == "drag-handle")
      ...
      //if target element is "resize" div
  ...
  maximize: function (t, e) {
    ...
    t.style.width = "100%";
    t.style.height = "100%";
    t.contentarea.style.height = "91.5%";
  },
  ...
  enablecontrols: function (e) {
    ...
    if (/Minimize/i.test(sourceobj.getAttribute("title")))
      ...
      //if this is the "maximize" control
  ...
  minimize: function (button, t) {
    ...
    t.style.width = "250px";
    var windowspacing = t.minimizeorder * 4; //spacing (gap) between each minmized window(s)
    t.style.top =
  ...
  restore: function (button, t) {
    ...
    t.contentarea.style.display = "block";
    if (t.resizeBool)
      //if this window is resizable, enable the resize icon
  ...
  close: function (t) {
    try {
      ...
      if (typeof closewinbol == "undefined") {
        alert(
          'An error has occured somwhere inside your "onclose" event handler'
        );
        var closewinbol = true;
      }
    }
    if (closewinbol) {
      ...
      if (t.state != "minimized")
        //if this window isn't currently minimized
        dhtmlwindow.rememberattrs(t); //remember window's dimensions/position on the page before closing
      if (window.frames["_iframe-" + t.id])
        //if this is an IFRAME DHTML window
  ...
  show: function (t) {
    if (t.isClosed) {
      alert(
        "DHTML Window has been closed, so nothing to show. Open/Create the window again."
      );
      return;
    }
    if (t.lastx)
      //If there exists previously stored information such as last x position on window attributes (meaning it's been minimized or closed)
  ...
  cleanup: function () {
    for (var i = 0; i < dhtmlwindow.tobjects.length; i++) {
      dhtmlwindow.tobjects[i].handle._parent =
        dhtmlwindow.tobjects[i].resizearea._parent =
        dhtmlwindow.tobjects[i].controls._parent =
          null;
    }
    window.onload = null;
  },
